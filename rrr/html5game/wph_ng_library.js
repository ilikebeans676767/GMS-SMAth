...
var slotLoadStatus   = -2;
var scoreBoardStatus = -1; // -1 = nothing, 0 = loading, 1 = loaded 
...
function ng_unlockmedal(medal_id) {
	...
	// Not fully loaded
	if(medals <= -1) {
		console.log("Medals not fully loaded");
		return;
	}
	...
	function ng_postScore(board_id, score_value) {
	    ...
	    console.log("Sending: ", score_value);
...
function ng_logEvent(eventname) {
	console.log("Logging Event: ", eventname);
	ngio.callComponent('Event.logEvent', {event_name:eventname, host:"www.newgrounds.com"});
}
...
function ng_logView(){
	ngio.callComponent('App.logView', {host:"www.newgrounds.com"});
}
...
function ng_ping() {
    ...
    // Send command to NG
    ngio.callComponent('Gateway.ping', {});
}
...
function ng_deleteSlot(slot_id) {
	...
	ngio.callComponent('CloudSave.clearSlot', { id: slot_id }, function (result) {
		if (result.success) {
			console.log(result);
		} else {
			console.log(result.error.message);
...
var arrayOfScores = [];
function ng_initializeTopScores(scoreboard_id, shownSize=5, startingIndex=0, showOnlyFriends=false,scorePeriod="W") {
	...
	ngio.callComponent('ScoreBoard.getScores', { 
		...
		period:scorePeriod, 
		skip:startingIndex, 
		social:showOnlyFriends
	}, onTopScoresLoaded);

}

function onTopScoresLoaded(result) {
	...
	// When the 
	if (result.success) {
		arrayOfScores = result.scores;
		scoreBoardStatus = 1;
		console.log("Successfully loaded top scores");
	} else {
		console.log("LOADING ERROR: ", result.error.message);
		scoreBoardStatus = -1;
    }
...
function ng_getJsonTopScores() {

	// Check if they are loaded
	if(scoreBoardStatus != 1) return "undefined";

	// Create an empty array to push all the results in later
	var resultJson = [];

	for(var i = 0; i < arrayOfScores.length; i++) {
		
		var arrayScore = arrayOfScores[i];
		
		// Put the results in a user dictionary then push in the array 
		var user = {
			userName:       arrayScore.user.name, 
			userId:         arrayScore.user.id,
			userScore:      arrayScore.value,
			userIconS:      arrayScore.user.icons.small,
			userIconM:      arrayScore.user.icons.medium,
			userIconL:      arrayScore.user.icons.large,
			userSupporter : arrayScore.user.supporter,
		};
		resultJson.push(user);
